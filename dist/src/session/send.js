...
export async function send(runner, options, messages, message) {
    ...
    const toolSet = activeTools.length > 0
        ? Object.fromEntries(activeTools.map(({ name, keywords: _keywords, ...rest }) => [name, rest]))
        : undefined;
    const updatedMessages = [...messages, { role: 'user', content: message }];
    const result = await queue.enqueue(options.scope ?? options.profile, () => generateText({
        model,
