...
export async function send(runner, options, messages) {
    ...
    const buildToolSet = (msgs) => {
        ...
        return activeTools.length > 0
            ? Object.fromEntries(activeTools.map(({ name, keywords: _keywords, ...rest }) => [name, rest]))
            : undefined;
    };
    const updatedMessages = messages.map((m) => typeof m === 'string' ? { role: 'user', content: m } : m);
    ...
    const result = await queue.enqueue(options.scope ?? options.profile, async () => {
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            ...
            try {
                const genResult = await generateText({
                    ...
                    prepareStep: async (ctx) => {
                        ...
                        if (typeof options.tools !== 'function')
                            return base;
                        return { ...base, tools: buildToolSet(ctx.messages) };
                    },
                    stopWhen: options.stopWhen ?? resolvedMaxSteps,
                    providerOptions: profile.providerOptions,
                    onStepFinish: async (step) => {
                        const timeoutMs = step.toolCalls.length > 0 && options.toolTimeoutMs != null
                ...
                const isTimeout = controller.signal.aborted && controller.signal.reason === 'request-timeout';
                const reason = isTimeout ? 'timeout' : (err?.name ?? 'error');
